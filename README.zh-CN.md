...
- **语音对话**：按 **Shift + 触发键** 即可与 ChatGPT 进行实时语音对话（Advanced Voice Mode），再次按触发键或 **Esc** 结束对话
...
1. 按住 **Shift** 同时按下**触发键**（如 Shift+Fn），即可与 ChatGPT 进行实时语音对话
2. ChatGPT 会通过语音回复你，无需文字粘贴
3. 再次按下**触发键**或按 **Esc** 结束对话
...
1. 点击菜单栏的 **MicPipe** 图标。
2. 找到 **Hotkey** 菜单项。
3. 从支持的按键中选择：**Fn**、**Command**、**Option** 或 **Control**。
